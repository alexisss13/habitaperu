  ...
  return (
    <div>
      <div className="py-8 px-6 max-w-7xl mx-auto">
        ...
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold tracking-tight text-text mb-2">Control y Gestión de Pagos</h1>
          <p className="text-base font-medium text-text-muted">
            Monitorea el recaudo mensual de tus propiedades alquiladas y valida los comprobantes recibidos.
          </p>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
          {filteredPayments.length > 0 ? (
              ...
              <tbody>
                {pagination.paginatedItems.map((p) => (
                  <tr key={p.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors text-xs">
                    <td className="p-4">
                      <p className="font-bold text-text truncate max-w-[180px]">{p.propertyName}</p>
                      <p className="text-[10px] text-text-muted mt-0.5">{p.propertyDistrict}</p>
                    </td>
                    <td className="p-4">
                      <p className="font-bold text-text">{p.tenantName}</p>
                      <p className="text-[10px] text-text-muted mt-0.5">{p.tenantEmail}</p>
                    </td>
                    ...
                    <td className="p-4 text-right">
                      ...
                      {p.status === "PAGADO" && (
                        <button
                          ...
                          className="h-9 px-3 border border-slate-200 text-slate-600 hover:bg-slate-50 rounded-lg font-bold text-xs cursor-pointer bg-white"
                        >
                          Ver Voucher
                        </button>
                      )}
                      {p.status === "PENDIENTE" && (
                        <span className="text-[10px] text-slate-400 font-semibold italic mr-2">Espera depósito</span>
                      )}
                    </td>
                  </tr>
                ))}
            ...
            <div className="text-center py-20 bg-white">
              <CreditCardIcon size={56} className="text-slate-300 mx-auto mb-4" />
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 shadow-2xl rounded-2xl max-w-lg w-full overflow-hidden">
            ...
            <div className="bg-slate-50 border-b border-slate-200 px-6 py-4 flex items-center justify-between">
              <div>
                <h3 className="text-base font-bold text-text">Revisar Pago del Inquilino</h3>
                <p className="text-xs text-text-muted mt-0.5">{reviewPayment.tenantName} • Cuota {getMonthName(reviewPayment.dueDate)}</p>
              </div>
              <button 
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-xs font-bold text-text bg-slate-50 p-4 rounded-xl">
                <div>
                  <span className="text-[10px] text-text-muted uppercase block mb-0.5">Monto Declarado</span>
              ...
              {reviewPayment.receipt && (
                <div className="space-y-1.5">
                  <span className="text-[10px] text-text-muted uppercase block font-bold">Voucher Enviado:</span>
              ...
              {reviewPayment.notes && (
                <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-100">
                  <span className="text-[9px] text-text-muted font-bold uppercase block mb-0.5">Comentarios del Inquilino:</span>
                  <p className="text-xs text-text font-normal leading-relaxed">{reviewPayment.notes}</p>
                </div>
              )}
              ...
              {rejectMode ? (
                ...
                <div className="pt-4 border-t border-slate-100 flex justify-between gap-3">
                  <button
                    ...
                    className="h-11 px-5 border border-red text-red hover:bg-red/5 rounded-xl text-xs font-bold transition-colors cursor-pointer bg-white"
                  >
      ...
      {viewPayment && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 shadow-2xl rounded-2xl max-w-lg w-full overflow-hidden">
            ...
            <div className="bg-slate-50 border-b border-slate-200 px-6 py-4 flex items-center justify-between">
              <div>
                <h3 className="text-base font-bold text-text">Voucher de Alquiler Verificado</h3>
                <p className="text-xs text-text-muted mt-0.5">{viewPayment.tenantName} • Cuota {getMonthName(viewPayment.dueDate)}</p>
              </div>
              <button 
            ...
            <div className="p-6 space-y-4 text-xs font-semibold text-text">
              <div className="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-xl">
                ...
                <div className="mt-2">
                  <span className="text-[10px] text-text-muted uppercase block mb-0.5">Confirmado el</span>
              ...
              {viewPayment.receipt && (
                <div className="space-y-1.5">
                  <span className="text-[10px] text-text-muted uppercase block">Comprobante de Pago:</span>
                  <div className="border border-slate-200 rounded-xl overflow-hidden relative h-56 bg-slate-100 flex items-center justify-center">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img 
              ...
              {viewPayment.notes && (
                <div>
                  <span className="text-[10px] text-text-muted uppercase block mb-1">Notas del Registro:</span>
                  <p className="text-text font-normal leading-relaxed">{viewPayment.notes}</p>
                </div>
              )}
      ...
      {reminderPayment && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 shadow-2xl rounded-2xl max-w-md w-full overflow-hidden">
            ...
            {/* Header */}
            <div className="bg-slate-50 border-b border-slate-200 px-6 py-4 flex items-center justify-between">
              <h3 className="text-base font-bold text-text">Recordatorio de Alquiler Vencido</h3>
              <button 
                ...
                <div className="space-y-4">
                  ...
                  <div className="pt-4 border-t border-slate-100 flex justify-end gap-3">
                    <button
                      type="button"
                      onClick={() => setReminderPayment(null)}
                      className="h-10 px-5 border border-slate-200 rounded-lg text-xs font-bold text-text-muted bg-white cursor-pointer"
                    >
                      Cancelar
                    </button>
                    <button
