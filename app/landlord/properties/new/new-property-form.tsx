...
export function NewPropertyForm() {
  ...
  const [images, setImages] = useState<string[]>(Array(MAX_PHOTOS).fill(""))
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    setLoading(true)
    const finalImages: string[] = images.map(u => u.trim()).filter(Boolean)
    try {
            ...
            <div className="space-y-6">
              ...
              <div className="pt-4 border-t border-slate-200">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button type="button" onClick={() => setImages(DEMO_IMAGES)}
                    className="text-[10px] font-bold text-accent hover:underline bg-transparent border-0 cursor-pointer">
                    Usar fotos de prueba
                  </button>
                </div>
                <p className="text-[10px] text-slate-400 -mt-2 mb-1">Sube hasta 5 fotos. La primera es obligatoria y será la portada del anuncio.</p>
                <div className="space-y-4">
                  {images.map((url, index) => {
                    const label = index === 0 ? "Foto 1 — Portada (Obligatoria)" : `Foto ${index + 1} (Opcional)`
